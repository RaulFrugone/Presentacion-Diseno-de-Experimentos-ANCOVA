...
Considerando que la estimación de los parámetros del modelo anterior por máxima verosimilitud. Así, la función de Máxima Verosimilitud queda expresada como:
...
$$
\mu : \hat{\mu} = \frac{y_{\bullet\bullet} - n \cdot \sum_{j=1}^t \hat{\tau}_j}{N} = \bar y_{..}
$$
$$
\tau_j :  \hat{\tau_j}= \frac{y_{\bullet j} - n\cdot \hat{\mu} -  \hat{\beta} \sum_{i=1}^n(x_{ij} - \bar{x}_{\bullet j})}{n} = \bar y_{.j}-\bar y_{..}-\beta(\bar x_{.j}-\bar x_{..})
$$

$$\beta : \hat{\beta} =  \frac{SCT_{xy}- \sum_{j=1}^t \hat{\tau}_j \sum_{i=1}^n(x_{ij} - \bar{x}_{\bullet j})}{SCT_{xx}} = 
                   \frac{\sum_{i=1}^n\sum_{j=1}^{t}(y_{ij}-\mu-\tau_j)(x_{ij}-\bar x_{..})}
...
$$\sigma^2: \hat{\sigma}^2
        = \frac{1}{N}
          \sum_{j=1}^t\sum_{i=1}^{n}
            \bigl(y_{ij}-\mu-\tau_j-\beta(x_{ij}-\bar x_{..})\bigr)^2$$
A partir de la siguiente condición $\sum_{j=1}^t \hat{\tau}_j=0$ obtenemos:

$\hat{\mu} = \bar{y}_{\bullet\bullet}$

$\hat{\tau} = \bar{y}_{\bullet j} - \bar{y}_{\bullet \bullet} - \hat{\beta}(x_{\bullet j} - x_{\bullet \bullet})$

Para $\beta$ lo obtenemos mediante:
...
$$
$$
\hat{y}_{ij}=\bar{y}_{\cdot\cdot} + \left[ \bar{y}_{j\cdot} - \bar{y}_{\cdot\cdot} - \hat{\beta}(\bar{x}_{j\cdot} - \bar{x}_{\cdot\cdot}) \right] + \hat{\beta}(x_{ij} - \bar{x}_{j\cdot})
$$

$$
\hat{y}_{ij}= \bar{y}_{j\cdot} + \hat{\beta}(x_{ij} - \bar{x}_{j\cdot})
$$

$$
\hat{y}_{ij}= \bar{y}_{j\cdot} + 0.954 \cdot (x_{ij} - \bar{x}_{j\cdot})
$$
